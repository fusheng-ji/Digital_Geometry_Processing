...
struct  AABB
{
    ...
    __attribute__((unused)) AABB(Vertex* vertex_list, int num);
...
private:
	...
	float point_tri_distance_refine(const Vector3f& p, Vector3f a, Vector3f b, Vector3f c, Vector3f &nearestP, float ref_dist);
