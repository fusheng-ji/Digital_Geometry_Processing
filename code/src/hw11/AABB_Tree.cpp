...
__attribute__((unused)) AABB::AABB(Vertex* vertex_list, int num)
{
    min = vertex_list[0];
    max = min;
    for (int i = 1; i < num; i++)
...
float AABB_Tree::point_tri_distance_refine(const Vector3f& p, Vector3f a, Vector3f b, Vector3f c, Vector3f &nearestP, float ref_dist)
{
